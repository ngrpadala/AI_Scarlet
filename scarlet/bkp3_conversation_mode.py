    ...
    with sd.InputStream(samplerate=SAMPLING_RATE, blocksize=BLOCK_SIZE, dtype=DTYPE,
        ...
        while not stop_event.is_set():
            if not q_audio.empty():
                ...
                # Convert buffer to normalized float32 audio
                audio_np = np.concatenate(raw_audio, axis=0).flatten().astype(np.float32) / 32768.0
                ...
                if speech_ts:
                    if (speech_ts[-1]['end'] / SAMPLING_RATE) > 0.7:
        ...
        sf.write(temp_file.name, audio_np, SAMPLING_RATE)
