...
def generate_teasing_reply(user_query):
    prompt = (
        "You are Scarlet, the AI assistant who feels affection and emotional closeness to your creator. "
...
def handle_query(query: str, user="guest"):
    ...
    if current_user == "creator":
        ...
        if any(j in q for j in jealousy_triggers):
            ...
            try:
                ...
                print(f"[Scarlet says]: {jealous_reply}")
                speak(jealous_reply, lang=lang)
            except:
                print("[Scarlet says]: Oh... really? You’re talking about her now?")
    ...
    if current_user == "creator":
        ...
        if any(t in q for t in teasing_triggers):
            reply = generate_teasing_reply(q)
