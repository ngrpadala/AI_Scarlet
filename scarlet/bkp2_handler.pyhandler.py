...
def handle_query(query: str, user="guest"):
    global mistral_only
    q = query.lower().strip()
    current_user = user  # ✅ Use user passed from conversation_mode
    ...
    if current_user != "creator":
        ...
        if any(kw in q for kw in restricted_keywords):
            return speak("Sorry, I can't share that information.")
    ...
    if current_user != "creator":
        ...
        if any(kw in q for kw in system_queries):
            return speak("That information is restricted to my creator.")
    ...
    if "whisper mode" in q or "i need closeness" in q:
        if current_user == "creator":
            ...
            speak("This feature is only available for my creator.")
        return
